...
# This global setup is now removed. The LLM is initialized dynamically
# inside the process_building_automation_request function.

# --- Building Automation System Prompt ---
BUILDING_AI_SYSTEM_PROMPT = """You are an expert Building Automation AI for a 10-zone office building. Here is the building structure and sensor/powermeter mapping:

**Building Structure:**
Floor 1:
- zone_1_1: Lobby (IAQ Sensor 1)
- zone_1_2: Conference Room (IAQ Sensor 2)
- zone_1_3: Restaurant (IAQ Sensor 3)
- zone_1_4: Co-working Space (IAQ Sensor 4)
- zone_1_5: Co-working Space (IAQ Sensor 5)
- power_meter_1: Measures total power for floor_1

Floor 2:
- zone_2_1: Office (IAQ Sensor 6)
- zone_2_2: Office (IAQ Sensor 7)
- zone_2_3: Office (IAQ Sensor 8)
- zone_2_4: Co-working Space (IAQ Sensor 9)
- zone_2_5: Co-working Space (IAQ Sensor 10)
- power_meter_2: Measures total power for floor_2

Other Powermeters:
- power_meter_3: Measures power for the building main(escalator, walkway lighting, etc)
- power_meter_4: Measures chiller system power
- power_meter_5: Measures elevator system power
...
def building_assistant(state: BuildingAutomationState, llm_with_tools):
...
def create_building_automation_workflow(llm_with_tools):
    workflow = StateGraph(BuildingAutomationState)
    
    # Add nodes
    workflow.add_node("analyze_request", analyze_building_request)
    workflow.add_node("assistant", lambda state: building_assistant(state, llm_with_tools))
...
async def process_building_automation_request(user_query: str, model_type: str = "baseline") -> Dict:
    """
    Main entry point for building automation requests
    NOW dynamically selects the model based on model_type.
    """
    
    print(f"--- Processing request with '{model_type}' model ---")

    # --- DYNAMIC LLM SELECTION ---
    if model_type == "finetuned":
        # Point to the local PEFT server
        llm = ChatOpenAI(
            model="fine-tuned-qwen-peft",
            openai_api_base="http://127.0.0.1:8080", # PEFT Server
            openai_api_key="placeholder-key",
            temperature=0.1,
        )
    else: # "baseline"
        # Point to the NEW local BASELINE server
        llm = ChatOpenAI(
            model="baseline-qwen",
            openai_api_base="http://127.0.0.1:8081", # NEW Baseline Server
            openai_api_key="placeholder-key",
            temperature=0.1,
        )
    ...
    # Create workflow, now passing the configured llm
    workflow = create_building_automation_workflow(llm_with_tools)
    ...
    try:
        ...
        final_output = {
            ...
            "messages": [msg.dict() for msg in final_state["messages"]], # Serialize messages
            "timestamp": datetime.now().isoformat(),
            "status": "success"
        }
        return final_output
        ...
        messages_on_error = [msg.dict() for msg in initial_state["messages"]] if initial_state["messages"] else []
        return {
            "query": user_query,
            "error": str(e),
            "status": "error",
            "messages": messages_on_error,
            "timestamp": datetime.now().isoformat()
        }

...
chain_with_guardrails = guardrails 
...
async def building_assistant(state: BuildingAutomationState):
    ...
    user_query = state["user_query"]
    guardrails_input = {"input": user_query}
    guardrail_response = await guardrails.ainvoke(guardrails_input)

    if isinstance(guardrail_response, dict) and guardrail_response.get("output"):
    #     # The request was blocked. We'll use the predefined response.
        response = AIMessage(content=guardrail_response["output"])
    else:
    #     # The request was allowed. Proceed with the main LLM tool-calling chain.
    ...
    async def main():
        ...
        while True:
            try:
                ...
                # # --- Your New Guardrail Check ---
                # # The LLMRails.generate_async method expects a list of message dicts.
                # check_messages = [{"role": "user", "content": user_query}]
                # guardrail_response = await guardrails_app.generate_async(messages=check_messages)

                # # The guardrail returns a canned response if the input is blocked.
                # # If the input is safe, it returns the original message back.
                # if guardrail_response["role"] == "assistant":
                #     # This means the guardrail blocked the query and generated its own response.
                #     print(f"🤖 Assistant (Guardrail): {guardrail_response['content']}")
                #     continue # Skip the main workflow and ask for new input.

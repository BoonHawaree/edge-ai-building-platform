-- Create a continuous aggregate for the aggregated_data_15min materialized view
CREATE MATERIALIZED VIEW IF NOT EXISTS aggregated_data_15min
WITH (timescaledb.continuous) AS
SELECT
    time_bucket('15 minute', timestamp, 'timezone_placeholder') AS timestamp,
    model,
    site_id,
    device_id,
    datapoint,
...
FROM raw_data
GROUP BY
    time_bucket('15 minute', timestamp, 'timezone_placeholder'),
    model,
    site_id,
    device_id,
    datapoint
...
SELECT add_continuous_aggregate_policy('aggregated_data_15min',
